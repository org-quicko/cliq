...
export class ProgramPromoterService {
    ...
    async getRandomProgramPromoter(programId: string) {
        ...
        if(!promoterMember){
            this.logger.error(`Error. Failed to get random programPromoter for program ID: ${programId}`);
            throw new NotFoundException(`Error. Failed to get random programPromoter for program ID: ${programId}`);
        }
