...
export class ApiKey {
    ...
    @ManyToOne(() => Program, (program) => program.apiKeys, { onDelete: 'CASCADE' })
    @JoinColumn({
        name: 'program_id',
    ...
    @CreateDateColumn({ name: 'created_at', default: () => `NOW()`, type: 'time with time zone', })
    createdAt: Date;

    @CreateDateColumn({ name: 'updated_at', default: () => `NOW()`, type: 'time with time zone', })
    updatedAt: Date;
