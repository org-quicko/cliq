...
export class ApiKeyController {
    ...
    @Post()
    async generateKey(@Headers('user_id') userId: string, @Param('program_id') programId: string) {
        ...
        return { message: `Successfully generated api key and secret for program ${programId}`, result };
    }
    ...
    @Patch(':api_key_id')
    async updateKey(@Param('program_id') programId: string, @Param('api_key_id') apiKeyId: string, @Body() body: UpdateApiKeyDto) {
        this.logger.info(`START: revokeKey controller`);

        await this.apiKeyService.updateKey(programId, apiKeyId, body);
        const keyStatusAction = body.status === statusEnum.ACTIVE ? 'reactivated' : 'revoked';
    ...
    @Delete(':api_key_id')
    async deleteKey(@Param('program_id') programId: string, @Param('api_key_id') apiKeyId: string) {
    ...
    async getAllKeys(@Param('program_id') programId: string) {
        ...
        return { message: `Successfully fetched all API keys of Program ${programId}.`, result };
    }
