    ...
    async canActivate(context: ExecutionContext) {
        ...
        if (key && secret) {
            ...
            if (apiKey) {
                ...
                this.logger.info(`END: canActivate function - AuthApiKeyGuard (authenticated via API Key)`);
                return true;
            }
        ...
        if (!authorization) {
            ...
            throw new UnauthorizedException('Missing authentication token and API key');
        }
        ...
        try {
            ...
            if (tokenPayload.aud === audienceEnum.PROGRAM_USER) {
                ...
                if (member) {
                    ...
                    this.logger.info(`END: canActivate function - AuthApiKeyGuard (authenticated member)`);
                    return true;
                }
            }
