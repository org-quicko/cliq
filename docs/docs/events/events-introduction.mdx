...
- A `programId` scoping the event to a specific referral program
...
Events are delivered asynchronously. Cliq will POST the event payload to each webhook URL registered for the matching event type under your program. Delivery is retried up to 3 times with exponential backoff on failure.
