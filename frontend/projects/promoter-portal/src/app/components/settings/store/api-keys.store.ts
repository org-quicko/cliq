...
export const ApiKeysStore = signalStore(
	...
	withDevtools('promoter-api-keys'),
	withMethods(
		(
			store,
			...
			fetchApiKey: rxMethod<{ programId: string; promoterId: string }>(
				pipe(
					tap(() => patchState(store, { status: Status.LOADING, error: null })),
					switchMap(({ programId, promoterId }) =>
						apiKeysService.fetchApiKey(programId, promoterId).pipe(
							tapResponse({
								next(response) {
									const apiKey = response.data
										? plainToInstance(ApiKeyDto, response.data)
										: null;
									patchState(store, {
										apiKey,
			...
			generateApiKey: rxMethod<{ programId: string; promoterId: string }>(
				pipe(
					tap(() => patchState(store, { status: Status.LOADING, error: null })),
					switchMap(({ programId, promoterId }) =>
						apiKeysService.generateApiKey(programId, promoterId).pipe(
							tapResponse({
								next(response) {
